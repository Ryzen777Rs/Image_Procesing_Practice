...
class SnippingAnnotator:
    ...
    def load_classes(self):
        ...
        if self.parent_app and hasattr(self.parent_app, "classes"):
            return [
                c
                for c in self.parent_app.classes
        ...
        if os.path.exists(self.classes_file):
            with open(self.classes_file, "r", encoding="utf-8") as f:
                ...
                return [
                    c for c in cls_list if c.lower() not in ["nothing", "all"]
                ]
        return []
    ...
    def _porneste_procedura_captura(self):
        ...
        if (
            ...
            and hasattr(self.parent_app.master, "is_running")
        ):
        ...
        threading.Thread(
            target=self._captureaza_ecran_background, daemon=True
        ).start()
        ...
        try:
            ...
            if self.parent_app:
                self.parent_app.after(
                    0,
                    lambda: self._deschide_interfata_cu_imagine(
                        original_img, None, None
                    ),
                )
            else:
    ...
    def open_annotation_ui(self, image_path=None, default_class=None):
        ...
        try:
            if image_path and os.path.exists(image_path):
                ...
                self._deschide_interfata_cu_imagine(
    ...
    def carca_adnotare_existenta(self, image_path, class_name):
        ...
        cale_txt = os.path.join(
            self.lbl_dir, class_name, f"{nume_fara_ext}.txt"
        )
    ...
    def create_fullscreen_window(self, bg_image):
        ...
        combo_values = ["nothing"] + [
            c for c in self.classes if c.lower() not in ["nothing", "all"]
        ]

        if (
            self.current_class_folder
            and self.current_class_folder.lower() not in ["all", "nothing"]
            and self.current_class_folder in combo_values
        ):
        ...
        self.btn_sterge_tot = ctk.CTkButton(
            self.top_panel,
            text="Șterge Tot",
            font=("Roboto", 13, "bold"),
            fg_color="#b53737",
    ...
    def _curata_eticheta_si_muta_in_all(self):
        if self.current_image_path:
            ...
            if self.current_class_folder and self.current_class_folder.lower() not in [
                "all",
                "nothing",
            ]:
                cale_txt = os.path.join(
            ...
            if self.current_class_folder and self.current_class_folder.lower() not in [
                ...
                cale_clasa = os.path.join(
                    self.img_dir,
                    self.current_class_folder,
                    base_filename + ".jpg",
                )
                if os.path.exists(cale_clasa) and os.path.abspath(
                    cale_clasa
    ...
    def save_annotation(self, event=None):
        ...
        if class_name not in self.classes:
            ...
            if self.parent_app and hasattr(self.parent_app, "creeaza_fila_clasa"):
                self.parent_app.after(
                    0, lambda: self.parent_app.creeaza_fila_clasa(class_name)
                )
        ...
        if self.current_image_path:
            ...
            if old_class and old_class != class_name:
                old_txt = os.path.join(
                    self.lbl_dir, old_class, base_filename + ".txt"
                )
        ...
        if len(self.points) >= 3:
            yolo_coords = [
                f"{x / self.width:.6f} {y / self.height:.6f}"
                for x, y in self.points
            ]
    ...
    def _reporneste_fluxul_principal(self):
        ...
        if self.parent_app:
            ...
            if hasattr(self.parent_app, "master") and hasattr(
                self.parent_app.master, "is_running"
            ):
